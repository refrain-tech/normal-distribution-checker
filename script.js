...
customElements.define("x-graph", HTMLGraphElement, {
	extends: "canvas"
});
...
function main(event) {
	 ...
	 * 5. 小さい順にソート
	 */
	const elements = o0wW1dZt.value.replace(/ /g, "").split(",").filter(element => isNumber(Number(element))).map(currentValue => parseFloat(currentValue)).sort((a, b) => a - b);
